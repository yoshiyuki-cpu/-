...
export default function EstimatesPage() {
  ...
  async function loadEstimates() {
    ...
    const withTotals = await Promise.all(data.map(async (e) => {
      const [{ data: wasteItems }, { data: extraItems }] = await Promise.all([
        supabase.from('estimate_waste_items').select('quantity, unit_price').eq('estimate_id', e.id),
        supabase.from('estimate_extra_items').select('amount').eq('estimate_id', e.id),
      ])
      const totals = calcEstimateTotals(e, wasteItems ?? [], extraItems ?? [])
