...
// 朝の KY活動・議事録を登録するまで、その現場のその日の人工・処分代（廃材）を入力させない仕組み。
...
export async function loadGate(supabase: SupabaseClient, projectId: number, date: string): Promise<GateState> {
  const base: GateState = { date, ky: false, minutes: false, unlockedBy: null, exempt: false, unknown: false }
  if (!date || date < GATE_START) return { ...base, exempt: true }
  try {
    const [ky, mn, un] = await Promise.all([
...
export function gateOpen(g: GateState) {
  return g.exempt || g.unknown || !!g.unlockedBy || (g.ky && g.minutes)
}

// 足りないものを「KY活動・議事録」のように並べる
export function gateMissing(g: GateState) {
  return [!g.ky && 'KY活動', !g.minutes && '議事録'].filter(Boolean).join('・')
}
